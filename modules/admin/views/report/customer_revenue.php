  ...
  <div class="col-lg-3">
    <div class="text-right">
      <?= Html::a("<i class='fas fa-file-csv'></i> Export CSV", [
        'report/export-customer-revenue', 'fromDate' => $fromDate, 'toDate' => $toDate
        ...
        <table class="table table-hover">
          <thead>
            <tr>
              ...
              <th width="20%" class="text-right">Unpaid</th>
            </tr>
          </thead>
          <tbody>
            <?php
            ...
            ?>
                <tr>
                  ...
                  <td class="text-right">
                    <div><strong><?= $value['total_amount'] ?></strong></div>
                    <?= "x{$value['total_invoice']} invoice(s)" ?>
                  </td>
                  <td class="text-right">
                    <div class="text-danger"><?= number_format($value['unpaid_amount'], 2) ?></div>
                  </td>
                </tr>
            <?php
              }
          ...
          <tfoot>
            <tr>
              <th class="text-right" colspan="2"><mark>Total Revenue</mark></th>
              <th class="text-right"><mark><?= $formater->DollarFormat(array_sum(array_column($data, 'total_amount'))) ?></mark></th>
              <th class="text-right"><span class="text-danger"><?= $formater->DollarFormat(array_sum(array_column($data, 'unpaid_amount'))) ?></span></th>
            </tr>
          </tfoot>
        </table>
      </div>
